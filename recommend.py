...
def find_closest(location, centroids):
    ...
    for element in centroids:
        a = distance(location, element)
        dist_cent.append(a)
    ind = dist_cent.index(min(dist_cent))
...
def group_by_centroid(restaurants, centroids):
    ...
    """
    # BEGIN Question 4
    ungrouped_list = [[find_closest(restaurant_location(restaurant), centroids), restaurant] for restaurant in restaurants]
    # END Question 4
    return group_by_first(ungrouped_list)
...
def find_centroid(cluster):
    ...
    loc_list = []
    for rest in cluster:
        loc_list.append(restaurant_location(rest))
    return [mean([x[0] for x in loc_list]),mean([x[1] for x in loc_list])]
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    pred = [find_predictor(user, reviewed, feature_fn) for feature_fn in feature_fns]
    best_pred, r_sq = max(pred, key = lambda x: x[1])
    # END Question 8
    return best_pred
